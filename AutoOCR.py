...
class Application():
    def __init__(self) -> None:
        ...
        self.title = tk.Label(self.main_window, text="Auto OCR", font=("HELVETICA",28,"bold"),bg=self.bg,fg=self.fg)
        self.title.grid(row=0,column=0,sticky=N,pady=(40,20),padx=40)
        self.desc = tk.Label(self.main_window, text="Click the button below and select the area of the question.",font=("ARIAL",12),bg=self.bg,fg=self.fg)
        self.desc.grid(row=1,column=0,padx=20, pady=20)
        self.snip_button = tk.Button(self.main_window,text="Select Area", font=("TIMES NEW ROMAN",14),bg="#FFFFFF",fg="#000000", height = 2, width = 12, command = self.snipImage, bd=7, relief=RAISED)
        ...
        self.ex.add_command(label="Copy Website",command=pyperclip.copy("https://sites.google.com/view/autoocrhelp/home/"))
        self.menu.add_cascade(label="Help",menu=self.ex)
    ...
    def takeBoundedScreenShot(self, x1, y1, x2, y2):
        ...
        pytesseractOut = pytesseract.image_to_string(path+".png")
        try:
            resp = search(query=pytesseractOut, stop=1, num=1, pause=2)
            for j in resp:
                pyperclip.copy(j)
        except:
            print("Error")
